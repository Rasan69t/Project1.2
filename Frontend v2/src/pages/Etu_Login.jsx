...
export default function Login() {
  ...
  // ── Champs connexion (2 champs uniquement) ───────────────────
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!controller.signal.aborted) setFilieres([]);
      } finally {
        if (!controller.signal.aborted) setLoadFilieres(false);
      }
    };
    load();
  ...
  // ── Connexion (étudiant OU admin, détecté côté backend) ─────
  // Le front envoie toujours : { nom, matricule, mot_de_passe }
  // avec la même valeur dans matricule et mot_de_passe.
  // Le backend cherche d'abord un admin par nom, sinon un étudiant.
  const handleConnexion = async (e) => {
    ...
    try {
      const res = await fetch(`${API}/auth/login`, {
        ...
        body: JSON.stringify({
          nom:          loginNom.trim(),
          matricule:    loginMatricule.trim(), // pour l'étudiant
          mot_de_passe: loginMatricule.trim(), // pour l'admin (même valeur, le backend fait la distinction)
        }),
      });
  ...
  const handleInscription = async (e) => {
    ...
    try {
      ...
      setSuccess("✅ Demande envoyée ! L'administrateur va valider votre compte. Revenez vous connecter une fois validé.");
        ...
        <div className="bg-[#2b5ce6] p-12 flex-col justify-between text-white hidden md:flex">
          ...
          <div>
            ...
            <p className="text-sm opacity-75 leading-relaxed">
              Accédez à toutes les épreuves disponibles, filtrées par matière et filière, en un instant.
            </p>
          </div>
          ...
          {tab === "connexion" && (
            <>
              ...
              <p className="text-xs text-gray-400 mb-6">
                Entrez votre <strong className="text-gray-500">nom</strong> et votre{" "}
                <strong className="text-gray-500">matricule</strong> pour accéder à vos épreuves.
              </p>
              ...
              <form onSubmit={handleConnexion} className="space-y-4">
                ...
                {/* Matricule
                    autoComplete="current-password" → le navigateur mémorise et propose
                    les valeurs saisies précédemment, comme demandé.
                    type="text" pour que l'étudiant voie ce qu'il tape. */}
                <div>
              ...
              {/* Aucun lien vers un espace admin — l'étudiant ne doit pas le savoir */}
            </>
          )}
              ...
              <form onSubmit={handleInscription} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <div className="relative">
                      <IconUser />
                      <input type="text" autoComplete="family-name" value={regNom}
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <IconUser />
                      <input type="text" autoComplete="given-name" value={regPrenom}
                ...
                <div>
                  ...
                  <div className="relative">
                    <IconCard />
                    <input type="text" autoComplete="off" value={regMatricule}
                      onChange={e => { setRegMatricule(e.target.value); setError(""); }}
                      placeholder="Ex : ETU-2024-001" required className={inputCls} />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <IconMail />
                    <input type="email" autoComplete="email" value={regEmail}
                ...
                <button
                  type="submit"
                  disabled={loading || loadFilieres || filieres.length === 0}
                  className="w-full py-3.5 mt-2 bg-[#2b5ce6] hover:bg-[#1e47c8] active:scale-[0.98] text-white font-semibold text-sm rounded-[10px] transition-all duration-200 disabled:opacity-60 flex items-center justify-center gap-2 cursor-pointer"
                >
